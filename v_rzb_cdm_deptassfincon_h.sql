create or replace view v_rzb_cdm_deptassfincon_h as
(
select *
  from (select  
               '发债'  htclass,
               ...
               a.vreserve7, --IRR
               b.dr
          from cdm_deptassfincon_h a --发债
          left join cdm_deptasscreditred_b b
            on a.pk_fundcontract = b.pk_fundcontract
          left join cdm_deptassobjmetter_b c
            on a.pk_fundcontract = c.pk_fundcontract
         where a.dr = '0')
         --存在a表有值，b表没值的情况，单据过滤一层b.dr
 where nvl(dr, 0) = '0')
;
